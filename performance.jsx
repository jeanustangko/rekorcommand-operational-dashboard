...
const PerfPage = (() => {
  ...
  // Coloring follows arrow direction everywhere: ▲ green, ▼ red.
  function DeltaChip({ delta, suffix = '%', periodLabel = 'prior period', decimals = 1 }) {
    const flat = Math.abs(delta) < Math.pow(10, -decimals) / 2;
    const cls = flat ? 'neutral' : delta > 0 ? 'good' : 'bad';
      ...
      <div className="op-bar-chart">
        ...
        <div className="op-bar-svg-wrap">
            ...
            <g key={i}>
              ...
              // Arrow direction drives color: ▲ green, ▼ red
              const deltaColor = Math.abs(deltaPct) < 0.05 ? 'var(--text-3)' : deltaPct > 0 ? '#16A34A' : '#DC2626';
              return (
                <g key={r.op.id}
  ...
  function OperatorDashboard() {
    ...
    // Arrow direction drives color everywhere: ▲ green, ▼ red
    const varianceClass = (v) => v > 0 ? 'good' : v < 0 ? 'bad' : 'neutral';
