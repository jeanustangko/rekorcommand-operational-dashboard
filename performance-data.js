...
(function () {
  ...
  // Agency-level rates. `delta` is the change vs the immediately prior
  // period — percentage POINTS for rates, PERCENT for timing/count metrics.
  const agencyKPIs = {
    confirmationRate: { avg: 87.4, median: 89.1, num: 612340, den: 700541, delta: 1.6 },
    rejectionRate:    { avg: 8.9,  median: 7.2,  num: 62348,  den: 700541, delta: -0.7 },
    completionRate:   { avg: 96.1, median: 97.4, num: 587412, den: 611320, delta: 0.4 },
    // Time between an incident being created in the system and being
    // confirmed by an operator. Reuses the first row of `workflow`.
    confirmationDelay: { avg: 1.8, median: 1.2, p90: 4.6, sample: 612340, delta: -8.2 },
    // Average number of status / note updates an operator posts per
    // confirmed incident over its lifetime.
    updateFrequency:   { avg: 4.6, median: 3.8, totalUpdates: 2818764, incidents: 612340, delta: 4.1 },
  };

  // Creation → Confirmation delay, broken down by shift (minutes).
  // `delta` is the percent change of the avg vs the prior period.
  const creationToConfirmationByShift = [
    { shift: 'Day',   window: '06–14', color: '#2563EB', avg: 1.4, median: 1.0, p90: 3.8, confirmed: 254120, delta: -11.4 },
    { shift: 'Mid',   window: '14–22', color: '#16A34A', avg: 1.9, median: 1.3, p90: 4.9, confirmed: 261480, delta: -4.2 },
    { shift: 'Night', window: '22–06', color: '#9333EA', avg: 2.4, median: 1.6, p90: 6.2, confirmed:  96740, delta: 6.8 },
  ];
