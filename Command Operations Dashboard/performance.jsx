...
const PerfPage = (() => {
  ...
  function RateCard({ title, info, kpi, color = 'var(--accent)' }) {
    return (
  ...
  function MetricCard({ title, info, kpi, unit, color = 'var(--accent)', context, barPct = 60, fmt }) {
  ...
  function ShiftDelayCompare({ data }) {
          ...
          return (
            <div key={s.shift} className={'sc-col ' + (isBest ? 'best' : '')}>
              ...
              <div className="sc-hero">
                ...
                <div className="sc-hero-label">Avg delay</div>
              </div>
        ...
        <>
            ...
            <div className="grid grid-12">
              ...
              info="Side-by-side comparison of how quickly operators on each shift confirm new incidents.">
                <ShiftDelayCompare data={shiftDelay} />
              </Card>
            </div>
  ...
  const OP_METRICS = [
  { key: 'confirmTime', label: 'Confirm time', unit: 'm', color: '#2563EB', accessor: (o) => o.confirmTime, fmt: (v) => v + 'm' },
  { key: 'avgUpdates', label: 'Avg updates / inc', unit: '', color: '#9333EA', accessor: (o) => o.avgUpdates, fmt: (v) => v.toFixed(1) },
  { key: 'completion', label: 'Completion rate', unit: '%', color: '#16A34A', accessor: (o) => o.completion, fmt: (v) => v + '%' },
  { key: 'rejection', label: 'Rejection rate', unit: '%', color: '#DC2626', accessor: (o) => o.rejection, fmt: (v) => v + '%' },
  { key: 'avgPerShift', label: 'Avg handled / shift', unit: '', color: '#F58220', accessor: (o) => Math.round(o.handled / o.scheduledDays), fmt: (v) => v.toLocaleString() },
  { key: 'variance', label: 'Vs previous period', unit: '%', color: '#0891B2', accessor: (o) => o.variance.handled, fmt: (v) => (v > 0 ? '+' : '') + v + '%', signed: true }];


  function OperatorBarChart({ data, selectedId, onSelect }) {
    const [metricKey, setMetricKey] = useState('confirmTime');
    const metric = OP_METRICS.find((m) => m.key === metricKey);
    const rows = data.map((o) => ({ op: o, v: metric.accessor(o) }));
    const vals = rows.map((r) => r.v);
    const maxV = Math.max(...vals, 0);
    const minV = Math.min(...vals, 0);
    const signed = metric.signed && minV < 0;
    const top = signed ? Math.max(Math.abs(maxV), Math.abs(minV)) : maxV;
    const yMax = top === 0 ? 1 : top * 1.15;

    const W = 1100,H = 260;
    const PAD = { l: 56, r: 16, t: 18, b: 64 };
    const innerW = W - PAD.l - PAD.r;
    const innerH = H - PAD.t - PAD.b;
    const zeroY = signed ? PAD.t + innerH / 2 : PAD.t + innerH;
    const barW = Math.max(14, Math.min(56, innerW / Math.max(rows.length, 1) - 8));
    const step = innerW / Math.max(rows.length, 1);

    const yToPx = (v) => {
      if (signed) return zeroY - v / yMax * (innerH / 2);
      return PAD.t + innerH - v / yMax * innerH;
    };

    const ticks = signed ?
    [-1, -0.5, 0, 0.5, 1].map((t) => +(t * yMax).toFixed(2)) :
    [0, 0.25, 0.5, 0.75, 1].map((t) => +(t * yMax).toFixed(2));
  ...
  function OperatorDashboard() {
    ...
    return (
      <>
        ...
        info="Visualize any operator metric across the operators in the current filter. Y-axis is the chosen metric; X-axis is selected operators. Click a bar to drill into that operator below.">
